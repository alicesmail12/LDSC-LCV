...
parser.add_argument('--sep', default=None, type=str, help="Separators in GWAS file: enter t for tsv, c for csv or s for space-delim file.")
...
def remove_MHC(args):
    ...
    if args.sep == 'c':
        sumstats = pd.read_csv(args.sumstatsFile, sep=',')
    if args.sep == 's':
        sumstats = pd.read_csv(args.sumstatsFile, sep='\s+')
    ...
    logging.info(sumstats.head())
    logging.info(list(sumstats.columns))
    ...
    if genomeBuild == 'GRCh37':
        sumstatsRm = sumstats.loc[((sumstats['CHROMOSOME'] == 6)|(sumstats['CHROMOSOME'] == 'chr6')) & (sumstats['POSITION']>28477797) & (sumstats['POSITION']<33448354)]
    ...
    if genomeBuild == 'GRCh38':
        sumstatsRm = sumstats.loc[((sumstats['CHROMOSOME'] == 6)|(sumstats['CHROMOSOME'] == 'chr6')) & (sumstats['POSITION']>28510120) & (sumstats['POSITION']<33480577)]
    ...
    sumstats.to_csv(args.outFile + ".tsv", sep='\t', encoding='utf-8', index=False)
    logging.info("\n*********************************************************************\n")
